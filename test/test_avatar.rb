...
class TestFakerAvatar < Test::Unit::TestCase
  ...
  def test_avatar_with_supported_format
    assert @tester.image(slug: "faker", format: "jpg").match(%r{https:\/\/robohash\.org\/faker\.jpg})
  end
  ...
  def test_avatar_with_set
    assert @tester.image(slug: "faker", format: "jpg", set: "set2").match(%r{https:\/\/robohash\.org\/faker\.jpg.*set=set2})
  end

  def test_avatar_with_bgset
    assert @tester.image(slug: "faker", format: "jpg", bgset: "bg1").match(%r{https:\/\/robohash\.org\/faker\.jpg.*bgset=bg1})
  end
end

...
class TestPlaceholdit < Test::Unit::TestCase
  ...
  def test_avatar_with_incorrect_format
    assert_raise ArgumentError do
      @tester.image(format: "wrong_format")
    end
  end

  def test_avatar_background_with_correct_six_char_hex
    assert @tester.image(background_color: "ffffff").match(%r{https:\/\/placehold\.it\/(.+)(png?)\/ffffff})
  end

  def test_avatar_background_with_correct_three_char_hex
    assert @tester.image(background_color: "fff").match(%r{https:\/\/placehold\.it\/(.+)(png?)\/fff})
  end

  def test_avatar_background_with_random_color
    assert @tester.image(background_color: :random).match(%r{https:\/\/placehold\.it\/(.+)(png?)\/[a-f0-9]{6}})
  end

  def test_avatar_background_with_wrong_six_char_hex
    assert_raise ArgumentError do
      @tester.image(background_color: "fffffz")
    end
  end

  def test_avatar_background_with_wrong_hex
    assert_raise ArgumentError do
      @tester.image(background_color: "ffff4")
    end
  end

  def test_avatar_background_with_wrong_three_char_hex
    assert_raise ArgumentError do
      @tester.image(background_color: "ffz")
    end
  end

  def test_avatar_font_color_with_correct_six_char_hex
    assert @tester.image(background_color: "ffffff", text_color: "000000").match(%r{https:\/\/placehold\.it\/(.+)(png?)\/ffffff\/000000})
  end

  def test_avatar_font_color_with_correct_three_char_hex
    assert @tester.image(background_color: "fff", text_color: "000").match(%r{https:\/\/placehold\.it\/(.+)(png?)\/fff})
  end

  def test_avatar_font_color_with_random_color
    assert @tester.image(background_color: "fff", text_color: :random).match(%r{https:\/\/placehold\.it\/(.+)(png?)\/fff\/[a-f0-9]{6}})
  end

  def test_avatar_font_color_with_wrong_six_char_hex
    assert_raise ArgumentError do
      @tester.image(text_color: "900F0z")
    end
  end

  def test_avatar_font_color_with_wrong_hex
    assert_raise ArgumentError do
      @tester.image(text_color: "x9")
    end
  end

  def test_avatar_font_color_with_wrong_three_char_hex
    assert_raise ArgumentError do
      @tester.image(text_color: "00p")
    end
  end

  def test_text_not_present
    assert @tester.image.match(%r{https:\/\/placehold\.it\/[^\\?]+$})
  end

  def test_text_present
    assert @tester.image(text: "hello").match(%r{https:\/\/placehold\.it\/(.+)\?text=hello})
  end
end
